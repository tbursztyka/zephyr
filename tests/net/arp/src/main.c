			     ...
			     struct net_eth_addr *hwaddr, u16_t type)
{
	...
	net_buf_add(pkt->frags, sizeof(struct net_eth_hdr));
}
...
void test_arp(void)
{
	...
	if (memcmp(net_pkt_ll_dst(pkt2)->addr, net_eth_broadcast_addr(),
		   sizeof(struct net_eth_addr))) {
		printk("ARP ETH dest address invalid\n");
		net_hexdump("ETH dest wrong  ", net_pkt_ll_dst(pkt2)->addr,
			    sizeof(struct net_eth_addr));
	...
	if (memcmp(net_pkt_ll_src(pkt2)->addr,
