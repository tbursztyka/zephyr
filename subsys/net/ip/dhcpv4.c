...
{
	if (net_udp_create(pkt, DHCPV4_CLIENT_PORT, DHCPV4_SERVER_PORT)) {
		return false;
	}
					     ...
					     bool server_id, bool requested_ip)
{
	...
	pkt = net_pkt_allocate_with_buffer(iface, size, AF_INET,
					   IPPROTO_UDP, K_FOREVER);

	net_pkt_set_ipv4_ttl(pkt, 0xFF);

	net_pkt_iter_init_to_data(pkt);

	/* op/htype/hlen/hops (0)/xid/secs/flags */
	if (net_pkt_write_u8(pkt, DHCPV4_MSG_BOOT_REQUEST) ||
	    net_pkt_write_u8(pkt, HARDWARE_ETHERNET_TYPE) ||
	    net_pkt_write_u8(pkt, HARDWARE_ETHERNET_LEN) ||
	    net_pkt_write_u8(pkt, 0) ||
	    net_pkt_write_be32(pkt, iface->config.dhcpv4.xid) ||
	    net_pkt_write_be16(pkt, 0) ||
	    net_pkt_write_be16(pkt, DHCPV4_MSG_BROADCAST)) {
		goto fail;
	}
	...
	if (ciaddr) {
		 ...
		 */
		if (net_pkt_write(pkt, (void *)ciaddr, 4)) {
			goto fail;
		}
	} else if (net_pkt_memset(pkt, 0, 4)) {
		goto fail;
	}

	/* yiaddr, siaddr and giaddr skipped / chaddr  */
	if (net_pkt_memset(pkt, 0, 12) ||
	    net_pkt_write(pkt, net_if_get_link_addr(iface)->addr,
			  net_if_get_link_addr(iface)->len) ||
	    net_pkt_memset(pkt, 0, CHADDR_PADDING_LEN)) {
		goto fail;
	}
 ...
 * as per RFC 2132.
 */
static bool dhcpv4_parse_options(struct net_if *iface,
				 struct net_pkt *pkt,
					 ...
					 void *user_data)
{
	...
	if (net_pkt_skip(pkt, NET_IPV4UDPH_LEN) ||
	    net_pkt_read(pkt, &op, 1) ||
	    /* htype/hlen/hops */
	    net_pkt_skip(pkt, 3) ||
	    net_pkt_read(pkt, &xid, sizeof(u32_t)) ||
	    /* secs/flags/ciaddr */
	    net_pkt_skip(pkt, 8) ||
	     /* yiaddr */
	    net_pkt_read(pkt, iface->config.dhcpv4.requested_ip.s4_addr,
			 sizeof(iface->config.dhcpv4.requested_ip.s4_addr)) ||
	    /* siaddr/giaddr */
	    net_pkt_skip(pkt, 8) ||
	    net_pkt_read(pkt, chaddr, sizeof(chaddr))) {
		return NET_DROP;
	}

	if (!(op == DHCPV4_MSG_BOOT_REPLY &&
	      iface->config.dhcpv4.xid == ntohl(xid) &&
	      !memcmp(chaddr, net_if_get_link_addr(iface)->addr,
		      net_if_get_link_addr(iface)->len))) {

		NET_DBG("Unexpected op (%d), xid (%x vs %x) or chaddr",
			op, iface->config.dhcpv4.xid, ntohl(xid));
		return NET_DROP;
	}
	...
	if (!dhcpv4_parse_options(iface, pkt, &msg_type)) {
